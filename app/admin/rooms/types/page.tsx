  ...
  return (
    <AdminLayout>
      <div className="space-y-6 max-w-[1540px] mx-auto pb-12 font-sans text-[#111923]">
        {/* 1. Page Header */}
        <div className="relative rounded-2xl border border-[#E8DFD2] bg-[#FCFAF6] p-7 sm:p-8 shadow-[0_4px_18px_rgba(40,30,20,0.03)] flex flex-col md:flex-row md:items-center justify-between gap-6 overflow-hidden">
          ...
          <div className="space-y-2 z-10">
            ...
            <div className="flex items-center space-x-3.5 pt-1">
              ...
              <h1 className="text-2xl sm:text-[32px] font-serif font-bold text-[#111923] tracking-tight leading-tight">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          {displayCategories.map((cat) => (
            ...
            >
              {/* Left Side: Room Image (42% width) */}
              <div className="relative w-full md:w-[42%] min-h-[240px] md:min-h-[320px] flex-shrink-0 bg-[#FAF7F2] overflow-hidden">
                <Image
                  src={cat.image}
                  alt={cat.name}
                  fill
                ...
                <div className="absolute top-3 left-3 bg-[#111923]/80 backdrop-blur-xs px-2.5 py-1 rounded-md text-[10px] uppercase tracking-wider font-bold text-white border border-white/10 shadow-xs">
                  {cat.badge}
                </div>

                {/* Top-Right Price Badge */}
                <div className="absolute top-3 right-3 bg-[#111923]/85 backdrop-blur-xs px-3 py-1 rounded-md text-xs font-bold text-white border border-[#B8893E]/40 shadow-xs">
              ...
              {/* Right Side: Room Information (58% width) */}
              <div className="flex-1 p-5 sm:p-6 flex flex-col justify-between space-y-4">
                <div className="space-y-3.5">
                  {/* Title & Description */}
                  <div className="space-y-1">
                    <h2 className="font-serif text-[23px] sm:text-[25px] font-bold text-[#111923] tracking-tight leading-snug group-hover:text-[#B8893E] transition-colors">
                      {cat.name}
                    </h2>
                    <p className="text-[12px] text-[#635C52] leading-relaxed line-clamp-2">
                      {cat.description}
                    </p>
                  </div>

                  {/* 3-Column Specifications Row */}
                  <div className="grid grid-cols-3 gap-2 bg-[#FCFAF6] p-2.5 sm:p-3 rounded-xl border border-[#EBE3D7] text-left">
                    <div className="flex items-center space-x-2">
                      ...
                      <div>
                        ...
                        <span className="text-[11.5px] font-bold text-[#111923] leading-tight block mt-0.5">
                          {cat.maxGuests}
                        </span>
                      </div>
                    </div>

                    <div className="flex items-center space-x-2 border-l border-[#EBE3D7] pl-2.5">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5 items-center">
                      {cat.amenities.map((a) => (
                        <span
                          key={a}
                          className="px-2.5 py-1 rounded-md bg-[#FCFAF6] border border-[#EAE2D5] text-[10.5px] text-[#554E44] font-medium flex items-center space-x-1"
                        >
                          <span className="text-[#A9824B] font-bold mr-1">✓</span>
                          <span>{a}</span>
                        </span>
                      ))}
                      {cat.moreAmenitiesCount > 0 && (
                        <span className="text-[10.5px] text-[#A9824B] font-bold px-1.5 py-1">
                ...
                <div className="pt-3.5 border-t border-[#EDE6DB] flex items-center justify-between gap-2">
                  <div className="flex items-center space-x-2">
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg border border-[#E2D8CA] bg-[#FCFAF6] hover:bg-[#F3EDE4] text-[#111923] text-[11px] font-semibold flex items-center space-x-2 transition-colors shadow-2xs cursor-pointer text-left"
                    >
                      <Eye className="w-3.5 h-3.5 text-[#B8893E] flex-shrink-0" />
                      <div className="leading-tight">
                        <span className="block text-[10.5px] font-medium text-[#111923]">Preview</span>
                        <span className="block text-[10px] font-bold text-[#6B6255]">Card</span>
                      </div>
                    </button>
                    ...
                    <Link
                      ...
                      className="px-3 py-1.5 rounded-lg border border-[#E2D8CA] bg-[#FCFAF6] hover:bg-[#F3EDE4] text-[#111923] text-[11.5px] font-semibold flex items-center space-x-2 transition-colors shadow-2xs"
                    >
                      <Calendar className="w-3.5 h-3.5 text-[#B8893E] flex-shrink-0" />
                      <span className="text-[11px] font-semibold text-[#111923]">Availability</span>
                    </Link>
                  </div>
                  ...
                  <button
                    ...
                    className="px-3.5 py-1.5 rounded-lg bg-[#A9824B] hover:bg-[#96713D] text-white flex items-center space-x-2 transition-all shadow-xs cursor-pointer active:scale-95 text-left"
                  >
                    <Edit3 className="w-3 h-3 flex-shrink-0" />
                    <div className="leading-tight uppercase tracking-wider font-bold">
                      <span className="block text-[9.5px]">CONFIGURE</span>
                      <span className="block text-[9.5px]">ROOM</span>
                    </div>
                  </button>
                </div>
              </div>
